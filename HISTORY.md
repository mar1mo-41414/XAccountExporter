...
## 2026-09-18 定期自動取得(cron)対応: `fetch-all`サブコマンド追加

ユーザーから「data/配下がそのままアカウントIDなので、それを回せば定期取得できそう。
cronで良いか」との相談。方針として、生のシェルループをcronに直接書くのではなく、
`xarchive fetch-all`サブコマンドを追加してツール側で一元管理する形にした
(レート制限設定やエラー時の継続処理を共通化できるため)。

- `data/`配下の各ディレクトリのうち`posts/`サブディレクトリを持つもの(＝取得済み
  アカウント)を列挙し、順番に差分`fetch`→`build`を実行する
- 多重実行防止として`data/.fetch-all.lock`に対して`fcntl.flock`の非ブロッキング
  排他ロックを取得する。取得できなければ(前回の実行が続いている)即座に終了コード1で
  抜ける。`fcntl`はPOSIX専用のため、importできない環境(Windows等)ではロック無しで
  続行するようフォールバックしてある(Windows版CLIは今のところこのコマンドの主要な
  利用想定ではないため)
- ロックファイルは(unlink競合を避けるため)実行後も削除せず、同じパスに対して
  毎回flockする方式にした(flockベースの排他ロックの定石)
- 実際に2プロセスを同時起動して2本目が正しく「既に実行中のためスキップ」で
  即終了することを確認した上で、1本目が5アカウント分を順に処理しきることも確認済み

cron設定はユーザー環境(marnux)側で`crontab`に直接登録(毎日4時、家庭内ネットワークの
負荷が低い時間帯とのこと)。ログは`data/fetch-all.log`に追記(`data/`は元々gitignore
対象なのでログがリポジトリに紛れ込むことはない)。

## 2026-09-19 check-deletedが削除を検知できないバグ

ユーザーが自分のテスト用アカウントで実際にツイートを1件削除し、`check-deleted`を
実行したが「削除は検知されませんでした」と誤判定された。実装時(2026-09-17)に
`--simulate`実行時の終了コード/出力を実機検証せず、`404`/`not found`等の文字列と
「終了コードが0でないこと」を条件に憶測で実装していたのが原因だった。

実際にgallery-dlで削除済みツイートのURL(`https://x.com/i/status/<id>`)を
`--simulate`した結果を確認したところ:
```
[twitter][info] No results for https://x.com/i/status/<id>
終了コード: 0
```
**削除済みでも終了コードは0(正常終了)**で、判定材料になるのは出力テキストの
`"No results for"`のみだった(`returncode != 0`という前提条件が常に偽になり、
